...
export const handleGetPosts = (req, res) => {
  ...
  if (author!== undefined) {
    if(typeof author !== "string" || author.trim() == "") {
      return res.status(400).json({ error: "Bad Request", message: "Author quer parameter is required"});
    }
    const filteredPosts = posts.filter((p) => p.author.toLowerCase() === author.toLowerCase());
